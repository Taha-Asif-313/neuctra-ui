...
const FloatingParticle: React.FC<FloatingParticleProps> = ({ delay = 0 }) => {
  const [windowHeight, setWindowHeight] = useState(1000); // fallback height
  const x = Math.random() * 100;
  const duration = 5 + Math.random() * 5;

  // ✅ Run only in browser to get window height
  useEffect(() => {
